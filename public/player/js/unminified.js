...
(function (window){
	...
	FWDEVPUtils.smpBtnNPos = function(){
		FWDEVPSimpleButton.prototype.hasTransform3d_bl = false;
		FWDEVPSimpleButton.prototype.hasTransform2d_bl = false;
	}

	FWDEVPUtils.cmpBtnNPos = function(){
		FWDEVPComplexButton.prototype.hasTransform3d_bl = false;
		FWDEVPComplexButton.prototype.hasTransform2d_bl = false;
	}
...
(function (window){
	...
	var FWDConsole = function(){
		...
		_s.setupScreen = function(){
			_s.main_do = new FWDEVPDisplayObject("div", "absolute");
...
(function(window){
	...
	var FWDEVPAudioScreen = function(prt, volume){
		...
		_s.init = function(){
			_s.setupAudio();
			//if(!FWDEVPUtils.isLocal) _s.setupSpectrum();
		};
		...
		_s.setupSpectrum = function(){
			...
			_s.canvas_do = new FWDEVPDisplayObject("canvas");
		...
		_s.renderSpectrum = function() {
			...
			for (var i = 0; i < _s.bars; i++) {
				...
				bar_height = Math.round(_s.fbc_array[i]/256 * limit)
				
				bar_width = Math.round(bar_height * 0.02);
								
				bar_x_term = _s.center_x + Math.cos(rads * i + _s.rot) * (_s.radius + bar_height);
				bar_y_term = _s.center_y + Math.sin(rads * i + _s.rot) * (_s.radius + bar_height);
...
var FWDEVPComplexButton = function(
		...
		_s.setupMainContainers = function(){
			if(_s.useFontAwesome_bl){
				_s.firstButton_do = new FWDEVPDisplayObject("div");
				_s.n1_do = new FWDEVPDisplayObject("div");	
				...
				//Second button
				_s.secondButton_do = new FWDEVPDisplayObject("div");
				_s.n2_do = new FWDEVPDisplayObject("div");	
				...
				if(_s.useHEX){
					...
					_s.n1_do = new FWDEVPDisplayObject("img");	
				...
				if(_s.allowToCreateSecondButton_bl){
					...
					if(_s.useHEX){
						_s.s1_do = new FWDEVPTransformDisplayObject("div");
						...
						_s.s1_do = new FWDEVPDisplayObject("img");
				...
				if(_s.useHEX){
					...
					_s.n2_do = new FWDEVPDisplayObject("img");	
				...
				if(_s.allowToCreateSecondButton_bl){
					...
					if(_s.useHEX){
						_s.s2_do = new FWDEVPTransformDisplayObject("div");
						...
						_s.s2_do = new FWDEVPDisplayObject("img");
...
(function (){
	var FWDEVPContextMenu = function(prt, _d){
		...
		_s.removeFromDOM = function(){
			//if(_s.prt.main_do.contains(_s)) _s.prt.main_do.removeChild(_s);
			_s.setX(-5000);
		};
...
var FWDEVPController = function(
		...
		_s.buttons_ar = [];
		_s.ytbQuality_ar = null;
		_s.ytbButtons_ar = null;
		
		_s.pointer_do;
		_s.ytbDisabledButton_do = null;
		_s.disable_do = null;
		_s.mainHolder_do = null;
		_s.ytbButtonsHolder_do = null;
		...
		_s.ytbQualityButton_do = null;
		_s.shareButton_do = null;
		_s.fullScreenButton_do = null;
		_s.ytbQualityArrow_do = null;
		_s.bk_do = null;
		...
		_s.init = function(){
			...
			if(_d.thumbnailsPreview) _s.setupThumbnailsPreview();
			if(_s.repeatBackground_bl){
				_s.bk_do = new FWDEVPDisplayObject("div"); 
				_s.bk_do.getStyle().background = "url('" + _s.controllerBkPath_str +  "')";
			}else{
				_s.bk_do = new FWDEVPDisplayObject("img");
				var img = new Image();
			...
			//_s.mainHolder_do.getStyle().backgroundColor = "#000000";
		...
		_s.setupDisable = function(){
			_s.disable_do = new FWDEVPDisplayObject("div");
			if(FWDEVPUtils.isIE){
		...
		_s.setupPlaybackRateButton = function(){
			if(_s.useVectorIcons_bl){
				FWDEVPSimpleButton.setPrototype();
				FWDEVPUtils.smpBtnNPos();
				var ic = 'fwdicon fwdicon-watch-later';
		...
		_s.updatePlaybackRateButtons = function(playbackRates, playbackRateIndex){
			if(!_s.playbackRateButton_do) return;
			//_s.playbackRateButton_do.enable();
			_s.positionAndResizePlaybackRateButtons(playbackRates);
			setTimeout(function(){
				//playbackRateIndex = _s.playbackRatesSource_ar.length - 1 - playbackRateIndex;
		...
		_s.setupPlaybackRateButtons = function(){
			...
			_s.playbackRatesButtonsHolder_do = new FWDEVPDisplayObject("div");
			...
			if(_s.repeatBackground_bl){
				...
				_s.playbackRatesButtonsBackground_do = new FWDEVPDisplayObject("img");
				var img = new Image();
				img.src = _s.controllerBkPath_str;
			...
			_s.playbackRatesPonter_do = new FWDEVPDisplayObject("img");
			...
			_s.playbackRateQualityArrow_do = new FWDEVPDisplayObject("img");
		...
		_s.disablePlaybackRateButtons = function(index){
			...
			if(curQualityLevel == "highres" || curQualityLevel == "hd1080" || curQualityLevel == "hd720" || curQualityLevel == "hd1440" || curQualityLevel == "hd2160"){
				_s.playbackRateButton_do.showDisabledState();
			}else{
				_s.playbackRateButton_do.hideDisabledState();
			}
			*/
		};
		...
		_s.showPlaybackRateButtons = function(animate){
			...
			//var finalY = parseInt(- _s.playbackRatesButtonsHolder_do.h - 6);
			var finalX = parseInt(_s.playbackRateButton_do.x + (parseInt(_s.playbackRateButton_do.w - _s.playbackRatesButtonsHolder_do.w)/2));
		...
		_s.hidePlaybackRateButtons = function(animate){
			...
			if(animate){
				//FWDAnimation.to(_s.playbackRatesButtonsHolder_do, .6, {y:_s.sH, ease:Expo.easeInOut});
				FWDAnimation.to(_s.playbackRatesButtonsHolder_do, .6, {y:prt.sH, ease:Expo.easeInOut});
			}else{
				FWDAnimation.killTweensOf(_s.playbackRatesButtonsHolder_do);
				//_s.playbackRatesButtonsHolder_do.setY(_s.sH);
				_s.playbackRatesButtonsHolder_do.setY(prt.sH);
			}
		...
		_s.setupAdsLines = function(linesAr){
			...
			if(!_s.linesHolder_do){
				_s.linesHolder_do = new FWDEVPDisplayObject("div");
			...
			if(_s.lines_ar){
				...
				for(var i=0; i<_s.lines_ar.length; i++){
					...
					line.getStyle().background = "url('" + _d.adLinePat_str + "') repeat-x";
		...
		_s.setupMainScrubber = function(){
			...
			_s.mainScrubber_do = new FWDEVPDisplayObject("div");
			_s.mainScrubber_do.setHeight(_s.scrubbersHeight);
			
			_s.mainScrubberBkLeft_do = new FWDEVPDisplayObject("img");
			_s.mainScrubberBkLeft_do.setScreen(_s.mainScrubberBkLeft_img);
			
			_s.mainScrubberBkRight_do = new FWDEVPDisplayObject("img");
			...
			//if(_s.isMobile_bl){
				_s.mainScrubberBkMiddle_do = new FWDEVPDisplayObject("div");	
				_s.mainScrubberBkMiddle_do.getStyle().background = "url('" + _s.mainScrubberBkMiddlePath_str + "') repeat-x";
			//}else{
				//_s.mainScrubberBkMiddle_do = new FWDEVPDisplayObject("img");
				//_s.mainScrubberBkMiddle_do.setScreen(middleImage);
			//}
			...
			//setup darg bar.
			_s.mainScrubberDrag_do = new FWDEVPDisplayObject("div");
			_s.mainScrubberDrag_do.setHeight(_s.scrubbersHeight);
		
			
			if(_s.useHEX){
				_s.mainScrubberDragLeft_do = new FWDEVPDisplayObject("div");
				...
				_s.mainScrubberDragLeft_do = new FWDEVPDisplayObject("img");
			...
			_s.volumeScrubberDragMiddle_do = new FWDEVPDisplayObject("div");
			
			if(_s.useHEX){
				_s.mainScrubberDragMiddle_do = new FWDEVPDisplayObject("div");
				_s.mainScrubberMiddleImage.onload = function(){
					...
					setTimeout(function(){
						_s.volumeScrubberDragMiddle_do.getStyle().background = "url('" + _s.mainSCrubberDragMiddleImageBackground.src + "') repeat-x";
					},50)
				}
			}else{
				_s.mainScrubberDragMiddle_do = new FWDEVPDisplayObject("div");	
			...
			_s.mainScrubberBarLine_do = new FWDEVPDisplayObject("img");
			...
			_s.live_do = new FWDEVPDisplayObject("div");
			...
			_s.ttm = new FWDEVPScrubberToolip(_s.mainScrubber_do, _d.scrubbersToolTipLabelBackgroundColor, _d.scrubbersToolTipLabelFontColor, test);
			_s.addChild(_s.ttm);
		...
		_s.updateThumbnailsPreview = function(localX, percentScrubbed){
			if(!_d.thumbnailsPreview) return;
		...
		_s.mainScrubberOnOverHandler =  function(e){
			...
			if(_d.thumbnailsPreview) _s.thumbnailsPreview_do.show();
		...
		_s.setupPlayPauseButton = function(){
			if(_s.useVectorIcons_bl){
				...
				var ic1 = 'fwdicon fwdicon-play';
				var ic2 = 'fwdicon fwdicon-pause';
				_s.playPauseButton_do = new FWDEVPComplexButton(undefined, undefined, undefined, undefined, true, undefined, undefined, undefined,
					"<span class='" + ic1 + "'></span>",
					"<span class='" + ic2 + "'></span>",
					"EVPMainButtonsNormalState",
					"EVPMainButtonsSelectedState"
		...
		_s.setupYtbButtons = function(){
			...
			var img = new Image();
			img.src = _s.ytbQualityButtonPointerPath_str;
			_s.pointer_do = new FWDEVPDisplayObject("img");
			...
			var img = new Image();
			img.src = _s.youtubeQualityArrowPath_str;
			_s.ytbQualityArrow_do = new FWDEVPDisplayObject("img");
		...
		_s.ytbQualityClick = function(e){
			//if(prt.videoType_str == FWDEVPlayer.VIDEO) _s.disableQualityButtons(e.target.label_str);
		...
		_s.setYtbQualityArrowPosition = function(target){
			var curY = 0;
			if(!target){
				curY = _s.ytbDisabledButton_do.y + Math.round((_s.ytbDisabledButton_do.h - _s.ytbQualityArrow_do.h)/2);
			}else{
				curY = target.y + Math.round((target.h - _s.ytbQualityArrow_do.h)/2);
			}
		...
		_s.showQualityButtons = function(animate){
			...
			//var finalY = parseInt(- _s.ytbButtonsHolder_do.h - 6);
			var finalX = parseInt(_s.ytbQualityButton_do.x + (parseInt(_s.ytbQualityButton_do.w - _s.ytbButtonsHolder_do.w)/2));
		...
		_s.hideQualityButtons = function(animate){
			...
			if(animate){
				//FWDAnimation.to(_s.ytbButtonsHolder_do, .6, {y:_s.sH, ease:Expo.easeInOut});
				FWDAnimation.to(_s.ytbButtonsHolder_do, .6, {y:prt.sH, ease:Expo.easeInOut});
			}else{
				FWDAnimation.killTweensOf(_s.ytbButtonsHolder_do);
				//_s.ytbButtonsHolder_do.setY(_s.sH);
				_s.ytbButtonsHolder_do.setY(prt.sH);
			}
		...
		_s.setupYoutubeQualityButton = function(){
			...
			if(_s.useVectorIcons_bl){
				...
				FWDEVPUtils.smpBtnNPos();
				var ic = 'fwdicon fwdicon-settings';
		...
		_s.setupSubtitleButton = function(){
			if(_s.useVectorIcons_bl){
				FWDEVPComplexButton.setPrototype();
				FWDEVPUtils.cmpBtnNPos();
				var ic1 = 'fwdicon fwdicon-CC';
				var ic2 = 'fwdicon fwdicon-CC-off';
				_s.subtitleButton_do = new FWDEVPComplexButton(undefined, undefined, undefined, undefined, true, undefined, undefined, undefined,
					"<span class='" + ic1 + "'></span>",
					"<span class='" + ic2 + "'></span>",
					"EVPMainButtonsNormalState",
					"EVPMainButtonsSelectedState"
		...
		_s.subtitleButtonMouseUpHandler = function(){
			/*
			if(_s.subtitleButton_do.currentState == 1){
				_s.dispatchEvent(FWDEVPController.SHOW_SUBTITLE);
				_s.subtitleButton_do.setButtonState(0);
			}else{
				_s.dispatchEvent(FWDEVPController.HIDE_SUBTITLE);
		...
		_s.setupSubtitleButtons = function(){
			...
			_s.subtitlesButtonsHolder_do = new FWDEVPDisplayObject("div");
			...
			if(_s.repeatBackground_bl){
				...
				_s.subtitlesButtonsBackground_do = new FWDEVPDisplayObject("img");
				var img = new Image();
				img.src = _s.controllerBkPath_str;
			...
			var img = new Image();
			img.src = _s.ytbQualityButtonPointerPath_str;
			_s.subtitlesPonter_do = new FWDEVPDisplayObject("img");
			...
			var img = new Image();
			img.src = _s.youtubeQualityArrowPath_str;
			_s.subtitleQualityArrow_do = new FWDEVPDisplayObject("img");
		...
		_s.disableSubtitleButtons = function(index){
			...
			/*
			if(curQualityLevel == "highres" || curQualityLevel == "hd1080" || curQualityLevel == "hd720" || curQualityLevel == "hd1440" || curQualityLevel == "hd2160"){
				_s.subtitleButton_do.showDisabledState();
			}else{
				_s.subtitleButton_do.hideDisabledState();
			}
			*/
		};
		...
		_s.showSubtitleButtons = function(animate){
			...
			//var finalY = parseInt(- _s.subtitlesButtonsHolder_do.h - 6);
			var finalX = parseInt(_s.subtitleButton_do.x + (parseInt(_s.subtitleButton_do.w - _s.subtitlesButtonsHolder_do.w)/2));
		...
		_s.hideSubtitleButtons = function(animate){
			...
			if(animate){
				//FWDAnimation.to(_s.subtitlesButtonsHolder_do, .6, {y:_s.sH, ease:Expo.easeInOut});
				FWDAnimation.to(_s.subtitlesButtonsHolder_do, .6, {y:prt.sH, ease:Expo.easeInOut});
			}else{
				FWDAnimation.killTweensOf(_s.subtitlesButtonsHolder_do);
				//_s.subtitlesButtonsHolder_do.setY(_s.sH);
				_s.subtitlesButtonsHolder_do.setY(prt.sH);
			}
		...
		_s.setupDownloadButton = function(){
			if(_s.useVectorIcons_bl){
				FWDEVPSimpleButton.setPrototype();
				FWDEVPUtils.smpBtnNPos();
				var ic = 'fwdicon fwdicon-download';
			...
			//_s.downloadButton_do.addListener(FWDEVPSimpleButton.SHOW_TOOLTIP, _s.downloadButtonShowToolTipHandler);
		...
		_s.setupChromecastButton = function(){
			if(_s.useVectorIcons_bl){
				FWDEVPComplexButton.setPrototype();
				FWDEVPUtils.cmpBtnNPos();
				var ic1 = 'fwdicon fwdicon-cast';
				var ic2 = 'fwdicon fwdicon-uncast';
				_s.ccBtn_do = new FWDEVPComplexButton(undefined, undefined, undefined, undefined, true, undefined, undefined, undefined,
					"<span class='" + ic1 + "'></span>",
					"<span class='" + ic2 + "'></span>",
			...
			//_s.buttons_ar.push(_s.ccBtn_do);
		...
		_s.setupFullscreenButton = function(){
			...
			if(_s.useVectorIcons_bl){
				FWDEVPComplexButton.setPrototype();
				FWDEVPUtils.cmpBtnNPos();
				var ic1 = 'fwdicon fwdicon-fullscreen';
				var ic2 = 'fwdicon fwdicon-normalscreen';
				_s.fullScreenButton_do = new FWDEVPComplexButton(undefined, undefined, undefined, undefined, true, undefined, undefined, undefined,
					"<span class='" + ic1 + "'></span>",
					"<span class='" + ic2 + "'></span>",
					"EVPMainButtonsNormalState",
					"EVPMainButtonsSelectedState"
		...
		_s.setupTime = function(){
			_s.time_do = new FWDEVPDisplayObject("div");
		...
		_s.setupVolumeButton = function(){
			if(_s.useVectorIcons_bl){
				...
				FWDEVPVolumeButton.prototype.hasTransform3d_bl = false;
				FWDEVPVolumeButton.prototype.hasTransform2d_bl = false;
		...
		_s.setupVolumeScrubber = function(){
			//setup background bar
			_s.volumeScrubber_do = new FWDEVPDisplayObject("div");
			_s.volumeScrubber_do.setHeight(_s.scrubbersHeight);
			
			_s.volumeScrubberBkLeft_do = new FWDEVPDisplayObject("img");
			_s.volumeScrubberBkLeft_do.setScreen(_s.volumeScrubberBkLeft_img);
			
			_s.volumeScrubberBkRight_do = new FWDEVPDisplayObject("img");
			...
			i//f(_s.isMobile_bl){
				_s.volumeScrubberBkMiddle_do = new FWDEVPDisplayObject("div");	
				_s.volumeScrubberBkMiddle_do.getStyle().background = "url('" + _s.volumeScrubberBkMiddlePath_str + "') repeat-x";
			//}else{
				//_s.volumeScrubberBkMiddle_do = new FWDEVPDisplayObject("img");
			...
			//setup darg bar.
			_s.volumeScrubberDrag_do = new FWDEVPDisplayObject("div");
			_s.volumeScrubberDrag_do.setHeight(_s.scrubbersHeight);
		
			
			if(_s.useHEX){
				_s.volumeScrubberDragLeft_do = new FWDEVPDisplayObject("div");
				...
				_s.volumeScrubberDragLeft_do = new FWDEVPDisplayObject("img");
			...
			_s.volumeScrubberBarLine_do = new FWDEVPDisplayObject("img");
		...
		_s.updateVolumeToolTip = function(){
			...
			/*var x = _s.volumeScrubber_do.x + prt.main_do.x;
			if(prt.displayType == FWDEVPlayer.STICKY){}*/
...
(function(window){
	...
	var FWDEVPData = function(props, playListElement, root){
		...
		_s.largePlayN_img = null;
		_s.fullScreenN_img = null;
		_s.ytbQualityN_img = null;
		_s.ytbQualityD_img = null;
		_s.shareN_img = null;
		_s.normalScreenN_img = null;
		_s.embedN_img = null;
		_s.embedColoseN_img = null;
		...
		_s.flashCopyToCBPath_str = null;
		_s.mainFolderPath_str = null;
		_s.bkMiddlePath_str = null;
		_s.hdPath_str = null;
		_s.youtubeQualityArrowPath_str = null;
		...
		_s.ytbQualityButtonPointerPath_str = null;
		_s.youtubeQualityButtonNormalColor_str = null;
		_s.youtubeQualityButtonSelectedColor_str = null;
		_s.controllerBkPath_str = null;
		_s.logoPosition_str = null;
		_s.logoPath_str = null;
		_s.shareAndEmbedTextColor_str = null;
		_s.inputBackgroundColor_str = null;
		_s.borderColor_str = null;
		_s.inputColor_str = null;
		_s.secondaryLabelsColor_str = null;
		_s.mainLabelsColor_str = null;
		_s.embedPathS_str = null;
		_s.embedWindowClosePathS_str = null;
		_s.embedWindowInputBackgroundPath_str = null;
		_s.embedCopyButtonNPath_str = null;
		_s.embedCopyButtonSPath_str = null;
		_s.sendButtonNPath_str = null;
		_s.sendButtonSPath_str = null;
		_s.embedWindowBackground_str = null;
		...
		_s.showEmbedButton_bl;
		_s.showShareButton_bl;
		_s.allowToChangeVolume_bl = true;
		_s.showContextMenu_bl = false;
		_s.autoPlay_bl = false;
		_s.showPoster_bl = false;
		_s.loop_bl = false;
		_s.showVolumeScrubber_bl = false;
		_s.showVolumeButton_bl = false;
		_s.showControllerWhenVideoIsStopped_bl = false;
		_s.showLogo_bl = false;
		_s.hideLogoWithController_bl = false;
		_s.isMobile_bl = FWDEVPUtils.isMobile;
		_s.hasPointerEvent_bl = FWDEVPUtils.hasPointerEvent;
		...
		_s.parseProperties = function(){
			...
			if(!_s.useHEX){
				if(_s.sknPth.indexOf("dark") != -1){
					_s.sknPth = "minimal_skin_dark"
				}else if(_s.sknPth.indexOf("white") != -1){
					_s.sknPth = "minimal_skin_white";
				}
			}
			*/
			...
			//if(_s.thumbnailsPreview) _s.showMainScrubberToolTipLabel_bl = false;
			...
			//if(FWDEVPlayer.useYoutube == "no" || _s.isMobile_bl) _s.showYoutubeQualityButton_bl = false;
			...
			//if(_s.subtitlesSource_ar && _s.subtitlesSource_ar.length == 1) _s.showSubtitleButton_bl = false;
...
(function (window){
	...
	var FWDEVPDisplayObject = function(type, position, overflow, display){
		...
		_s.hasTransform3d_bl =  FWDEVPUtils.hasTransform3d;
		_s.hasTransform2d_bl =  FWDEVPUtils.hasTransform2d;
		if(FWDEVPUtils.isFirefox || FWDEVPUtils.isIE) _s.hasTransform3d_bl = false;
		if(FWDEVPUtils.isFirefox || FWDEVPUtils.isIE) _s.hasTransform2d_bl = false;
...
(function (window){
	...
	var FWDEVPInfo = function(prt, warningIconPath, showErrorInfo){
		...
		this.init = function(){
			...
			_s.textHolder_do = new FWDEVPDisplayObject("div");
			if(!FWDEVPUtils.isIEAndLessThen9) _s.textHolder_do.getStyle().font = "Arial";
...
(function (window){
	...
	var FWDEVPlayer = function(props){
		...
		_s.init = function(){
			...
			_s.flash_do = null;
			_s.flashObject = null;
			_s.videoPoster_do = null;
			_s.largePlayButton_do = null;
			_s.hider = null;
			_s.embedWindow_do = null;
			...
			_s.isPlaying_bl = false;
			_s.firstTapPlaying_bl = false;
			_s.stickOnCurrentInstanceKey_bl = false;
		...
		_s.setupMainDo = function(){
			...
			_s.main_do = new FWDEVPDisplayObject("div", "relative");
			...
			if(_s.displayType ==  FWDEVPlayer.STICKY){
				_s.background_do = new FWDEVPDisplayObject("div");
				_s.background_do.getStyle().width = "100%";
				if(_s.mainBackgroundImagePath_str){
					_s.mainBackground_do =  new FWDEVPDisplayObject("div");
		...
		_s.startResizeHandler = function(){
			...
			if(_s.displayType == FWDEVPlayer.STICKY){
				if(FWDEVPUtils.isAndroid) window.addEventListener("orientationchange", _s.orientationChange);
		...
		//	if(!_s.orintationChangeComplete_bl) return;
			...
			}else if(_s.isFullScreen_bl || _s.displayType == FWDEVPlayer.FULL_SCREEN || _s.displayType == FWDEVPlayer.BACKGROUND_VIDEO){	
				_s.main_do.setX(0);
				_s.main_do.setY(0);
				_s.sW = viewportSize.w + 2;
				_s.sH = viewportSize.h + 2;
			...
			if(_s.fillEntireVideoScreen_bl  && _s.videoType_str == FWDEVPlayer.VIDEO){
				...
				if(_s.videoScreen_do && _s.videoScreen_do.video_el && _s.videoScreen_do.video_el.videoWidth != 0){
					...
					var scaleX = _s.tempVidStageWidth/originalW;
					var scaleY = _s.tempVidStageHeight/originalH;
					...
					var finalW = parseInt(originalW * totalScale) + 2;
					var finalH = parseInt(originalH * totalScale) + 2;
					var finalX = parseInt((_s.sW - finalW)/2) - 1;
					var finalY = parseInt((_s.sH - finalH)/2) - 1;
					
					_s.videoScreen_do.setWidth(finalW); 
					_s.videoScreen_do.setHeight(finalH); 
					_s.videoScreen_do.setX(finalX); 
					_s.videoScreen_do.setY(finalY);
					if(_s.main_do.alpha == 0){
			...
			if(_s.dClk_do){
				if(_s.is360 && _s.videoType_str == FWDEVPlayer.YOUTUBE){
					...
					if(_s.isMobile_bl){
						...
						//if(_s.videoType_str == FWDEVPlayer.YOUTUBE && !_s.isAdd_bl){
						//	_s.dClk_do.setHeight(_s.sH - 120);
						//}else{
							_s.dClk_do.setHeight(_s.sH);
						//}
					}
		...
		_s.setupClickScreen = function(){
			_s.dClk_do = new FWDEVPDisplayObject("div");
			
			if(_s.disableDoubleClickFullscreen_bl){
				_s.dClk_do.getStyle().pointerEvents = 'none';
			}
			...
			if(_s.displayType !=  FWDEVPlayer.BACKGROUND_VIDEO){
				if(_s.hasPointerEvent_bl){
					...
					//_s.dClk_do.screen.addEventListener("touchstart", _s.playPauseDownHandler);
		...
		_s.setupVisualization = function(){
			...
			_s.circle_do = new FWDEVPTransformDisplayObject('div');
			...
			_s.vzImg1_do = new FWDEVPTransformDisplayObject('img');
			...
			_s.vzImg2_do = new FWDEVPTransformDisplayObject('img');
			...
			_s.vzImg3_do = new FWDEVPTransformDisplayObject('img');
		...
		_s.addVisualization = function(pos){
			...
			//FWDAnimation.to(_s.circle_do, 1, {alpha:0});
			...
			if(typeof Vimeo != "undefined" && Vimeo.Player){
				...
				tag.onerror = function(){
					setTimeout(function(){
						...
						_s.info_do.showText("Error loading Vimeo API");
		...
		_s.setupYoutubeAPI = function(){
			if(_s.ytb_do || _s.isYoutubeAPiCreated_bl) return;
			 _s.isYoutubeAPiCreated_bl = true;
			if(typeof YT != "undefined" && YT.Player){
				...
				tag.onerror = function(){
					setTimeout(function(){
						_s.main_do.addChild(_s.info_do);
						_s.info_do.allowToRemove_bl = false;
						_s.info_do.showText("Error loading Youtube API");
						_s.preloader_do.hide(false);
						_s.preloader_do.stopPreloader();
					}, 500);
					return;
		...
		_s.setupData = function(){
			...
			_s._d.addListener(FWDEVPData.VAST_LOADING, _s.vastLoaded);
		...
		_s.setupAPT = function(){
			if(!_s.apt && _s._d.autoPlayText && _s._d.autoPlay_bl){
				_s.apt = new FWDEVPTransformDisplayObject('div');
		...
		_s.setupLargePlayPauseButton = function(){
			...
			_s.largePlayButton_do.hide(false);
		...
		_s.showEmbedWindowHandler = function(){
			...
			/*if(location.protocol.indexOf("file:") != -1){
				_s.main_do.addChild(_s.info_do);
				_s.info_do.showText("Embedding video files local is not allowed or possible! To function properly please test online.");
				return;
			}*/
		...
		_s.setupAudioScreen = function(){	
			...
			//_s.audioScreen_do.addListener(FWDEVPAudioScreen.START, _s.audioScreenStartHandler);
		...
		_s.videoScreenSafeToScrubbHandler = function(){
			...
			//if(_s.hasHlsPlayedOnce_bl && _s.videoType_str == FWDEVPlayer.HLS_JS) return;
			...
			if(_s.controller_do){
				...
				if(!_s.isAdd_bl){
					...
					if((_s.controller_do.thumbnailsPreview_do && _s._d.thumbnailsPreview != 'auto')
					|| ((_s.controller_do.thumbnailsPreview_do && _s._d.thumbnailsPreview == 'auto')
			...
			_s.showClickScreen();
			if(_s.fillEntireVideoScreen_bl && _s.videoScreen_do.video_el){
				_s.fillScreenId_int = setInterval(function(){
					 if(_s.videoScreen_do.video_el.videoWidth != 0){
					 	 _s.resizeHandler();
		...
		_s.videoScreenStopHandler = function(e){
			...
			if(_s.controller_do){
				...
				if(!_s._d.showControllerWhenVideoIsStopped_bl){
					_s.controller_do.hide(!_s.isMobile_bl, true);
				}else{
					_s.controller_do.show(!_s.isMobile_bl);
				}
				_s.hider.stop();
			}
			...
			if(_s.isMobile_bl && _s.videoType_str == FWDEVPlayer.YOUTUBE){
				_s.videoPoster_do.hide();
				if(_s.largePlayButton_do) _s.largePlayButton_do.hide();
			}
			_s.hider.reset();
		...
		_s.videoScreenPlayHandler = function(){
			...
			if(_s.logo_do && _s.videoType_str != FWDEVPlayer.VIMEO && !_s.useWithoutVideoScreen_bl) _s.logo_do.show(true);
		...
		_s.videoScreenPauseHandler = function(){
			...
			if(_s.largePlayButton_do && !_s._d.showAnnotationsPositionTool_bl && !_s.useWithoutVideoScreen_bl) _s.largePlayButton_do.show();
			if(_s.controller_do) _s.controller_do.show(true);
			if(_s.logo_do && _s.videoType_str != FWDEVPlayer.VIMEO && !_s.useWithoutVideoScreen_bl && !_s.useWithoutVideoScreen_bl) _s.logo_do.show(true);
			_s.hider.stop();
			_s.hider.reset();
			_s.showCursor();
		...
		_s.videoScreenUpdateTimeHandler = function(e, e2, e3, stopHandler){
			if(_s.isCasting) return;
			//if(_s.addSource_str && _s.addSource_str.indexOf("youtube.") != -1 && !_s.isYoutubeReady_bl) return;
			...
			if(!_s.isAdd_bl) _s.updateAds(seconds);
		...
		_s.positionAds = function(animate){
			...
			//if(_s.adsStart_do.isShowed_bl){
				...
				//logger.log(_s._d.adsButtonsPosition_str)
				
				_s.adsStart_do.setX(finalX);
			//}
			
			//if(_s.adsSkip_do.isShowed_bl){
				if(_s._d.adsButtonsPosition_str == "left"){
					finalX = 0;
				}else{
					finalX = _s.sW;
		...
		_s.onKeyDownHandler = function(e){
			...
			if (e.keyCode == 32){
				...
				if(_s.isCasting){
					...
					_s.flashObject.togglePlayPause();
				}
				if(e.preventDefault) e.preventDefault();
				return false;
		...
		_s.hiderShowHandler = function(){
			...
			if(_s.logo_do && _s._d.hideLogoWithController_bl && _s.isPlaying_bl && _s.videoType_str != FWDEVPlayer.VIMEO && !_s.useWithoutVideoScreen_bl) _s.logo_do.show(true);
			_s.showCursor();
			if(_s.isAdd_bl && _s._d.showSkipButton_bl){
				_s.positionAds(true);
		...
		_s.play = function(){
			...
			if(_s.videoType_str == FWDEVPlayer.YOUTUBE && !_s.isYoutubeReady_bl){
				...
				if(_s.largePlayButton_do && !_s.useWithoutVideoScreen_bl) _s.largePlayButton_do.show();
				return;
			}
			...
			if(_s.videoType_str == FWDEVPlayer.HLS_JS){
				...
				//if(!_s.isMobile_bl && !_s.isHLSManifestReady_bl) return;
			}
			...
			if(_s.isMobile_bl && _s.videoType_str == FWDEVPlayer.YOUTUBE && _s.ytb_do && !_s.ytb_do.isSafeToBeControlled_bl && !_s._d.autoPlay_bl) return;
			...
			if(_s.isIMA){
				...
				if(_s.vimeo_do.isStopped_bl){
					_s.startVimeoVideoWithDelay = setTimeout(_s.vimeo_do.play, 1000);
				}else{
					_s.vimeo_do.play();
				}
			}else if(_s.videoType_str == FWDEVPlayer.MP3){
		...
		_s.stop = function(source){
			...
			clearTimeout(_s.startVimeoVideoWithDelay);
			...
			//if(_s.controller_do && _s.controller_do.downloadButton_do) _s.controller_do.downloadButton_do.disable();
			...
			if(_s.isMobile_bl){
				if(source && source.indexOf(".") != -1){
					...
					if(!source && _s.videoType_str == FWDEVPlayer.VIDEO){
						_s.videoPoster_do.show();
						if(_s.largePlayButton_do && !_s.useWithoutVideoScreen_bl) _s.largePlayButton_do.show();
				...
				if(_s._d.showControllerWhenVideoIsStopped_bl && _s.controller_do_do) _s.controller_do.show(true);
				_s.videoPoster_do.show();
				if(_s.largePlayButton_do && !_s.useWithoutVideoScreen_bl) _s.largePlayButton_do.show();
			}
		...
		_s.setPosterSource = function(path){
			if(!_s.isAPIReady_bl || !path) return;
			...
			if(_s.videoSourcePath_str.indexOf(".") == -1 && _s.useYoutube_bl && _s.isMobile_bl){
				_s.videoPoster_do.setPoster("youtubemobile");
			}else{
				_s.videoPoster_do.setPoster(_s.posterPath_str);
				if(_s.prevPosterSource_str != path) _s.dispatchEvent(FWDEVPlayer.UPDATE_POSTER_SOURCE);
			}
		...
		_s.updateImageScreen = function(source){
			...
			if(!_s.imageSceeenHolder_do){
				_s.imageSceeenHolder_do = new FWDEVPDisplayObject("div");
			...
			_s.imageScreen_do = new FWDEVPDisplayObject("img");
		...
		_s.setSource = function(source, overwrite, videoType, source2){
			...
			if(_s.videoSourcePath_str.indexOf("vimeo.com") != -1 &&
			   source.indexOf(".mp4") == -1 && source.indexOf(".m3u8") == -1){
				...
				//if(_s.controller_do && !_s._d.showDefaultControllerForVimeo_bl)  _s.controller_do.setX(-5000);
				...
				if(_s.controller_do) _s.controller_do.setX(0);
			}else if(_s.videoSourcePath_str.toLowerCase().indexOf(".mp3") != -1){
			...
			if(_s.videoSource_str.indexOf("youtube.") != -1 && !_s.isYoutubeReady_bl){
				...
				if(location.protocol.indexOf("file:") != -1 && FWDEVPUtils.isIE){
					...
					_s.info_do.showText("This browser dosen't allow the Youtube API to run local, please test it online or in another browser like Firefox or Chrome.");
			...
			if(source.indexOf("vimeo.") != -1 && !_s.vimeo_do && _s.videoType_str == FWDEVPlayer.VIMEO){
				...
				if(location.protocol.indexOf("file:") != -1){
					//_s.info_do.allowToRemove_bl = false;
					_s.main_do.addChild(_s.info_do);
					_s.info_do.showText("This browser dosen't allow playing Vimeo videos local, please test online.");
					_s.resizeHandler();
					return;
				}
			...
			if(_s.is360 && !_s.isThreeJsOrbigLoaded_bl){
				...
				if(FWDEVPUtils.isLocal){
					_s.main_do.addChild(_s.info_do);
					_s.info_do.showText("This browser dosen't allow playing 360 videos local, please test online.");
					_s.preloader_do.hide(false);
					_s.preloader_do.stopPreloader();
					return;
				}
				...
				if(!FWDEVPUtils.hasWEBGL){
					_s.main_do.addChild(_s.info_do);
					_s.info_do.showText("Playing 360 videos in this browser is not possible because it dosen't support WEBGL.");
					_s.preloader_do.hide(false);
					_s.preloader_do.stopPreloader();
					return;
				}
				...
				if(!_s.isThreeJsLoaded_bl && !FWDEVPlayer.hasThreeJsLoaded_bl){
					...
					script.onerror = function(){
						_s.main_do.addChild(_s.info_do);
						_s.info_do.showText("Error loading 360 degree library <font color='#FF0000'>" + _s._d.threeJsPath_str + "</font>.");
						_s.preloader_do.hide(false);
						_s.preloader_do.stopPreloader();
						return;
					}
					...
					script.onload = function () {
						...
						script2.onerror = function(){
							_s.main_do.addChild(_s.info_do);
							_s.info_do.showText("Error loading three.js from <font color='#FF0000'>" + _s._d.threeJsControlsPath_str + "</font>.");
							_s.preloader_do.hide(false);
							_s.preloader_do.stopPreloader();
							return;
						}
			...
			if(_s.videoType_str == FWDEVPlayer.VIMEO){
				...
				if(_s.ytb_do && _s.ytb_do.ytb){
					//_s.ytb_do.ytb.a.style.visibility = "hidden";
					_s.ytb_do.showDisable();
				}
								
				_s.videoScreen_do.setVisible(false);
				
				//if(!_s.isMobile_bl) _s.vimeo_do.showDisable();
			...
			if(_s.videoType_str == FWDEVPlayer.YOUTUBE && _s.ytb_do &&  _s.ytb_do.ytb &&  _s.ytb_do.ytb.cueVideoById){
				...
				//if(_s.ytb_do.ytb) return;	
				if(_s.ytb_do && _s.ytb_do.ytb){
					//_s.ytb_do.ytb.a.style.visibility = "visible";
					if(_s.disableDoubleClickFullscreen_bl) _s.ytb_do.hideDisable();
				}
				...
				if(_s.isMobile_bl){
					...
					_s.playSecondSource = false;
					//if(_s.playYoutubeIfLoadedLate_bl && !_s.isMobile_bl) _s.play();	
				}
		...
		_s.goFullScreen = function(){
			...
			if(_s.isMobile_bl) window.addEventListener("touchmove", _s.disableFullScreenOnMobileHandler);
		...
		_s.setVideoSource =  function(source, videoType, isLive, source2){
			if(!_s.isAPIReady_bl) return;
			_s.isAdd_bl = false;
			...
			_s.setSource(source, false, videoType);
		};
...
(function (window){
	...
	var FWDEVPLightBox = function(
		...
		_s.setupMainContainers = function(){
			...
			_s.lightBoxBackground_sdo = new FWDEVPDisplayObject("div"); 
			...
			_s.mainLightBox_do = new FWDEVPDisplayObject("div");
			//_s.mainLightBox_do.getStyle().boxShadow = "0px 0px 5px #999999";
...
(function (window){
	...
	var FWDEVPLogo = function(
		...
		_s.init = function(){
			...
			_s.logoImage_do = new FWDEVPDisplayObject("img");
...
(function (){
	var FWDEVPOpener = function(prt, _d, position_str, playerIsShowed_bl){
		...
		_s.setupStuff = function(e){
			...
			if(_s.useHEX){
				_s.openN_do = new FWDEVPDisplayObject("div");
				...
				_s.openN_do = new FWDEVPDisplayObject("img");
			...
			if(_s.useHEX){
				_s.openS_do = new FWDEVPDisplayObject("div");
				...
				_s.openS_do = new FWDEVPDisplayObject("img"); 
			...
			if(_s.useHEX){
				_s.closeN_do = new FWDEVPDisplayObject("div");
				...
				_s.closeN_do = new FWDEVPDisplayObject("img");
			...
			if(_s.useHEX){
				_s.closeS_do = new FWDEVPDisplayObject("div");
				...
				_s.closeS_do = new FWDEVPDisplayObject("img"); 
...
var FWDEVPPopupAddButton = function(
		...
		_s.onLoadHandler = function(){
			...
			_s.image_do = new FWDEVPDisplayObject("img");
		...
		_s.hide = function(remove){
			...
			finalY = parseInt(prt.prt.tempVidStageHeight);
			prt.setY(finalY);
		...
		_s.show = function(){
			...
			setTimeout(function(){
				...
				if(_s.type == 'adsense' && !_s.isGooglAdCreated_bl){
					...
					_s.container = new FWDEVPTransformDisplayObject("div");
					...
					_s.ifr = new FWDEVPTransformDisplayObject("IFRAME");
					...
					if(_s.link){
						_s.clicker = new FWDEVPDisplayObject('div');
				...
				if(prt.prt.controller_do.isShowed_bl){
					finalY = parseInt(prt.prt.tempVidStageHeight - prt.prt.controller_do.h - (_s.originalH * scale) + 2 + finalH);
				}else{
					finalY = parseInt(prt.prt.tempVidStageHeight - (_s.originalH * scale) + 2 + finalH);
				}	
				prt.setY(finalY);
...
var FWDEVPPopupAds = function(prt, _d){
		...
		_s.update = function(duration){
			...
			for(var i=0; i<_s.totalAds; i++){
				...
				//console.log(popupAdButton.start + ' ' + popupAdButton.end)
...
(function (window){
	...
	var FWDEVPPreloader2 = function(imageSource_img, segmentWidth, segmentHeight, totalSegments, animDelay){
		...
		_s.init = function(){
			...
			_s.image_sdo = new FWDEVPDisplayObject("img");
...
var FWDEVPSimpleButton = function(nImg, 
		...
		_s.setupMainContainers = function(){
			if(_s.useFontAwesome_bl){
				_s.setOverflow('visible');
				_s.n_do = new FWDEVPDisplayObject("div");	
				_s.n_do.hasTransform3d_bl = false;
				_s.n_do.hasTransform2d_bl = false;
				...
				if(_s.showHDIcon){
					...
					_s.hd_do = new FWDEVPDisplayObject("img");
				...
				if(_s.useHEX && !_s.showOver){
					_s.n_do = new FWDEVPTransformDisplayObject("div");
					...
					_s.n_do = new FWDEVPTransformDisplayObject("img");	
	...
	FWDEVPSimpleButton.setPrototype = function(){
		...
		FWDEVPSimpleButton.prototype = new FWDEVPDisplayObject("div");
	};
...
var FWDEVPSimpleSizeButton = function(
		...
		_s.setupMainContainers = function(){
			...
			if(_s.useHEX && !_s.showOver){
				...
				_s.n_do = new FWDEVPDisplayObject("img");
				_s.n_do.setScreen(_s.nImg);
				_s.n_do.setWidth(_s.buttonWidth);
				_s.n_do.setHeight(_s.buttonHeight);
				_s.addChild(_s.n_do);
			...
			if(_s.useHEX){
				_s.s_do = new FWDEVPTransformDisplayObject("div");
				...
				_s.s_do = new FWDEVPDisplayObject("img");
	...
	FWDEVPSimpleSizeButton.MOUSE_UP = "onClick";
	...
	var FWDEVPTransformDisplayObject = function(type, position, overflow, display){
		...
		_s.addChildAtZero = function(e){
			if(_s.numChildren == 0){
				...
				_s.screen.insertBefore(e.screen, _s.children_ar[0].screen);
				if(_s.contains(e)){_s.children_ar.splice(FWDEVPUtils.indexOfArray(_s.children_ar, e), 1);}	
...
(function(window){
	...
	var FWDEVPVideoScreen = function(prt, backgroundColor_str, volume){
		...
		_s.stop = function(overwrite){
			...
			//logger.log("# VID stop #" + prt.instanceName_str);
			_s.isPlaying_bl = false;
			_s.isStopped_bl = true;
			_s.hastStaredToPlayHLS_bl = false;
			_s.hasPlayedOnce_bl = true;
		...
		_s.addGreenScreen = function(){
			if(!_s.canvasGR2){
				_s.canvasGR1 = new FWDEVPDisplayObject('canvas');
				_s.contextGR1 = _s.canvasGR1.screen.getContext('2d');
				_s.canvasGR2 = new FWDEVPDisplayObject('canvas');
				_s.contextGR2 = _s.canvasGR2.screen.getContext('2d');
			}
			//_s.addChild(_s.canvasGR2);
			_s.video_el.style.visibility = "hidden";
			_s.renderFR();
		}
		...
		_s.add360Vid = function(){
			...
			//document.documentElement.appendChild(_s.canvas);
...
(function(window){
	...
	var FWDEVPYoutubeScreen = function(prt, volume){
		...
		_s.controllerHeight = prt._d.controllerHeight;
		_s.hasHours_bl = false;
		_s.hasBeenCreatedOnce_bl = false;
		_s.allowScrubing_bl = false;
		_s.hasError_bl = false;
		_s.isPlaying_bl = false;
		_s.isStopped_bl = true;
		...
		_s.stateChangeHandler = function(e){
			//if(logger) logger.log(e.data + " " + prt.instanceName_str + " " + _s.isCued_bl)
			/*
			if(e.data == -1 && _s.isCued_bl && _s.isMobile_bl){
				
				if(_s.isStopped_bl){
					_s.isStopped_bl = false;
				}
				FWDEVPlayer.stopAllVideos(prt);
			}'*/
		...
		_s.stop = function(){
			if(_s.isStopped_bl) return;
			//if(logger) logger.log("# YTB stop #" + prt.instanceName_str);
