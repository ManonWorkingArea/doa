...
function preLoadExamPre() 
{
    var code    = "ZuMhJnUyvlmBsb3bKelT3rtfQjxSWT";
    var token   = Cookies.get('__session');
    var exam    = localStorage.getItem('__exam_pre');
    var access  = localStorage.getItem('__access');
    var access  = JSON.parse(access);
    ...
    if (exam == undefined) 
    {
        ...
        $.ajax(
        {
            ...
            success: function(result) {
                ...
                localStorage.setItem('__exam_pre', examArray);
                //Question
                var questionArray     = JSON.stringify(result.question);
                localStorage.setItem('__question_pre', questionArray);
                $.isLoading( "hide" );
            },
            error: function(request,msg,error) {
                //console.log("Error: " + error); //just use the err here
...
function renderQuestionPre()
{
    var course      = localStorage.getItem('__course');
    var course      = JSON.parse(course);

    var question    = localStorage.getItem('__question_pre');
    ...
    $.each(question, function (key, contest) 
    {
        ...
        $.each(contest.answer, function (key, answer) 
        {
            ...
            +"<label for='" + answer.id + "'>"
                +"<div class='check'><input type='radio' id='" + answer.id + "' name='" + contest.id + "' value='" + answer.id + "' data-root='" + contest.id + "' data-inner='" + answer.id + "'></div>"
        ...
        ).find('input').change(function()
        {
            ...
            window.localStorage.setItem(root, inner);
            getCurrentScore()
        });
        
        setAnswer(contest.id)
        quizArray.push(contest.id)
    });
...
function renderAccessPre() 
{
    ...
    $.isLoading({text: "กำลังโหลดข้อมูลกรุณารอสักครู่ ..."});
    localStorage.removeItem("__exam_pre");
    localStorage.removeItem("__question_pre");
    localStorage.removeItem("__access");
...
function checkAccessPre()
{
    ...
    if (d > 0){
        ...
        if(access.access_status==="denied"){
            ...
            console.log("Open");
            window.location.href="exam_pre.php?token=ZuMhJnUyvlmBsb3bKelT3rtfQjxSWT";
        }
    }
}
...
function preLoadExamFinal() 
{
    var code    = "kJVNdUpeEVYBeZt3tgcuQqgzwsm771";
    var token   = Cookies.get('__session');
    var exam    = localStorage.getItem('__exam_final');
    var access  = localStorage.getItem('__access');
    var access  = JSON.parse(access);
    ...
    if (exam == undefined) 
    {
        ...
        $.ajax(
        {
            ...
            success: function(result) {
                ...
                //Exam
                var examArray     = JSON.stringify(result.exam);
                localStorage.setItem('__exam_final', examArray);
                //Question
                var questionArray     = JSON.stringify(result.question);
                localStorage.setItem('__question_final', questionArray);
                $.isLoading( "hide" );
            },
            error: function(request,msg,error) {
                //console.log("Error: " + error); //just use the err here
...
function renderExamContentFinal() 
{
    var code    = $.urlParam('token');
    var token   = Cookies.get('__session');
    var exam    = localStorage.getItem('__exam_final');
...
function renderQuestionFinal()
{
    ...
    var question    = localStorage.getItem('__question_final');
    var question    = JSON.parse(question);
...
function renderAccessFinal() 
{
    ...
    $.isLoading({text: "กำลังโหลดข้อมูลกรุณารอสักครู่ ..."});
    localStorage.removeItem("__exam_final");
    localStorage.removeItem("__question_final");
    localStorage.removeItem("__access");
    ...
    if (exam == undefined) 
    {
        ...
        $.ajax(
        {
            ...
            success: function(result) {
                ...
                //g("__exam ajax: " + localStorage.getItem('__exam')); //just use the err here 
...
function accessLobby()
{
    ...
    $.ajax(
    {
        ...
        success: function(result) {
            //console.log("Approve");
            renderCourseContent();
            // $(".lobby-score-text").html(result.score);
        },
        error: function(request,msg,error) {
            //console.log("Error");
            window.location.href = "student.php";
